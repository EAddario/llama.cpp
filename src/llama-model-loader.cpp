...
const char * llama_ftype_name(llama_ftype ftype) {
    ...
    switch ((enum llama_ftype) (ftype & ~LLAMA_FTYPE_GUESSED)) {
        ...
        case LLAMA_FTYPE_MOSTLY_IQ3_K:     name = LLAMA_FTYPE_PREFIX "IQ3_K - 3.4375 bpw"; break;
...
static ggml_backend_buffer_type_t select_weight_buft(const llama_hparams & hparams, ggml_tensor * tensor, ggml_op op, const buft_list_t * buft_list) {
    ...
    for (const auto & cur : *buft_list) {
        ...
        if (ggml_backend_dev_type(cur_dev) == GGML_BACKEND_DEVICE_TYPE_GPU) { skipped_gpu = true; }
    }

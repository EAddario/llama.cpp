...
static const ggml_type all_types[] = {
    ...
    GGML_TYPE_IQ2_K, GGML_TYPE_IQ3_K, GGML_TYPE_IQ4_K,
...
static bool test_backend(ggml_backend_t backend, ggml_backend_dev_t dev, test_mode mode, const char * op_names_filter, const char * params_filter,
                         printer * output_printer, const char * test_file_path, int parallel_workers, bool strict) {
    ...
    if (mode == MODE_TEST) {
        ...
        std::atomic<size_t> n_not_supported = 0;
        ...
        // 0/0 tests passed reads as PASS so a run could mask a missing or unsupported type
        bool ran_any = true;
        if (strict && tests_run == 0) {
            printf("  no test case ran: %zu generated, %zu not supported\n", test_cases.size(), (size_t) n_not_supported);
            ran_any = false;
        }
...
static void usage(char ** argv) {
    ...
    printf(" [--show-coverage] [--test-file <path>] [-j <n>] [--strict]\n");
    printf("    valid modes:\n");
    ...
    printf("    --strict fails a run in which no test case ran, instead of reporting OK (test mode only)\n");
}
...
int main(int argc, char ** argv) {
    ...
    for (size_t i = 0; i < ggml_backend_dev_count(); i++) {
        ...
        bool ok = test_backend(backend.get(), dev, mode, op_names_filter, params_filter, output_printer.get(), test_file_path, parallel_workers, strict);
